...
class TestUndeclaredNameDegrades:
    def test_an_undeclared_name_degrades_and_logs(self, caplog) -> None:
        """A raise here would re-arm consolidation forever without billing.

        ``HistoryConsolidator._consolidate`` catches it with ``billed`` still
        False, so no backoff attempt is recorded and every idle tick retries.
        """
        _write_config({"memory_stores": {"default": {}, "work": {}}})
        with caplog.at_level(logging.WARNING, logger="kiro_crew.memory_stores"):
            resolved = memory_store_dir_for("no-such-store")
        assert resolved == workspace_dir()
        assert any(
            "no-such-store" in r.getMessage() and "not a declared" in r.getMessage()
            for r in caplog.records
        )

    def test_the_first_hop_is_the_configured_default(self, caplog) -> None:
        _write_config({"memory_stores": {"work": {}}, "default_memory_store": "work"})
        with caplog.at_level(logging.WARNING, logger="kiro_crew.memory_stores"):
            resolved = memory_store_dir_for("no-such-store")
        assert resolved == memory_stores_root() / "work"
        assert any("'work'" in r.getMessage() for r in caplog.records)

    def test_the_second_hop_is_the_literal_default_and_logs_twice(self, caplog) -> None:
        _write_config({"memory_stores": {"work": {}}, "default_memory_store": "also-missing"})
        with caplog.at_level(logging.WARNING, logger="kiro_crew.memory_stores"):
            resolved = memory_store_dir_for("no-such-store")
        assert resolved == workspace_dir()
        messages = [r.getMessage() for r in caplog.records]
        assert any("no-such-store" in m for m in messages)
        assert any("also-missing" in m for m in messages)

    def test_a_malformed_configured_default_is_stepped_over(self, caplog) -> None:
        """The second hop must not compose a path out of a bad name either."""
        _write_config({"memory_stores": {"work": {}}, "default_memory_store": "Bad_Name"})
        with caplog.at_level(logging.WARNING, logger="kiro_crew.memory_stores"):
            assert memory_store_dir_for("no-such-store") == workspace_dir()


# ---------------------------------------------------------------------------
# The shape rule (item 5) — this one raises
# ---------------------------------------------------------------------------
...
class TestMalformedAndDeclaredIsUndeclaredForResolution:
    ...
    def test_a_crew_bound_to_a_malformed_declared_store_degrades(self, caplog) -> None:
        """The binding degrades, so no malformed name reaches a resolver."""
        from kiro_crew.config.loader import resolve_agent_bindings

        _write_config(
            {
                "agents": {"crew": {"kiro_agent": "kirocrew", "memory_store": "Bad_Name"}},
                "default_agent": "crew",
                "memory_stores": {"Bad_Name": {}, "work": {}},
                "default_memory_store": "work",
            }
        )
        cfg = KiroCrewConfig.load()
        assert "Bad_Name" in cfg.memory_stores, "the operator's entry must survive the load"
        with caplog.at_level(logging.WARNING, logger="kiro_crew.memory_stores"):
            bindings = resolve_agent_bindings(cfg, agent_name="crew")
        assert bindings.memory_store_name == "work"
        assert any("Bad_Name" in r.getMessage() for r in caplog.records)

    def test_the_binding_never_hands_a_resolver_a_name_it_would_raise_on(self) -> None:
        """The whole point: the runtime raise is unreachable from a config."""
        from kiro_crew.config.loader import resolve_agent_bindings

        _write_config(
            {
                "agents": {"crew": {"kiro_agent": "kirocrew", "memory_store": "../escape"}},
                "default_agent": "crew",
                "memory_stores": {"../escape": {}},
                "default_memory_store": "../escape",
            }
        )
        cfg = KiroCrewConfig.load()
        name = resolve_agent_bindings(cfg, agent_name="crew").memory_store_name
        assert name == DEFAULT_MEMORY_STORE
        # No raise, and the resolved paths are the default store's own.
        assert memory_store_dir_for(name) == workspace_dir()
        assert resolve_store_path(name) == config_dir() / MEMORY_DB_FILE
        assert memory_index_path_for(name) == config_dir() / INDEX_DB_FILE
