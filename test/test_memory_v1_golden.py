...
def test_memory_is_injected_on_the_first_message_only(seeded: Seeded) -> None:
    ...
    # The follow-up is the user's text plus the interactive [OPTIONS:] reminder and
    # nothing else — no session-context wrapper, so no request header either.
    assert follow_up.startswith("and what about the rollback?\n\n(If presenting choices,")
    assert "[CURRENT USER REQUEST" not in follow_up
...
def test_seeded_home_file_set_is_pinned(seeded: Seeded) -> None:
    ...
    """
    history = str(seeded.history_file.relative_to(seeded.home))
