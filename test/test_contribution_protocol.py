...
def _grant(monkeypatch, *, app=APP, events=("demoapp/*",), projections=("demoapp/*",),
    ...
    monkeypatch.setattr("kiro_crew.apps.manager.get_app_manifest", lambda n: manifest if n == app else None)
...
class TestManifestDeclaration:
    def test_round_trips_and_validates(self):
        ...
        assert AppManifest.from_dict(m.to_dict()).contributions.to_dict() == m.contributions.to_dict()
    ...
    def test_an_undeclared_manifest_produces_the_pre_field_payload(self):
        ...
        raw = {"name": APP, "version": "1.0.0", "displayName": "D", "description": "d", "signer": "s"}
...
class TestExternalProjectionStore:
    ...
    def test_an_older_state_version_is_refused(self):
        ...
        with pytest.raises(ContribError) as exc:
            store.publish("member", SLUG, "demoapp/count", app=APP, value=2, seq=99, state_version=2)
        assert exc.value.code == "stale_seq"

    def test_rows_survive_a_restart(self, tmp_path):
        get_store().publish("member", SLUG, "demoapp/count", app=APP, value=5, seq=1, state_version=1)
...
class TestPublishRoute:
    ...
    async def test_publish_stores_and_pushes_the_kind_frame(self, tmp_path, monkeypatch):
        ...
        assert pushed == [
            (types.WS_MEMBER_PROJECTION, {"slug": SLUG, "key": "demoapp/count", "value": {"n": 3}, "seq": 2})
        ]
...
class TestSurfaceGrants:
    ...
    def test_member_frames_stay_owner_only(self, tmp_path, monkeypatch):
        ...
        _grant(monkeypatch)
        assert not wes.ws_event_allowed(
            types.WS_MEMBER_PROJECTION, {"slug": SLUG}, app=APP,
            allowed_events=frozenset({"*"}), state=state,
        )
