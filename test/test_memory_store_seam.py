...
# The seven that remain all share one property, and it is the reason they are
...
#
#   slack/gateway.py  cron, sequential and parallel — a `CronJob` carries no crew
#                     binding at all; `agent_id` / `agent_sequence` are kiro agent
#                     names passed straight to `get_or_create`
#   slack/gateway.py  the heartbeat — one process-wide `HEARTBEAT_KEY` on the fixed
#                     `kirocrew-heartbeat` template
#   dashboard/handlers/hooks.py  the webhook runner — its `agent` is validated
#                     against INSTALLED kiro templates (`_installed_agent_names`),
#                     and its `hook:` session is ephemeral, so it records no binding
#   task_planner.py / task_executor.py  the task runner — `agent` is a template id,
#                     and the per-step session keys are synthesized rather than
#                     bound; giving a run the store of the conversation it was
#                     started FROM is a design decision about whose memory a task
#                     run belongs to, not a resolution of identity already in scope
...
def _resolver_calls(gate: ModuleType, path: str) -> list[ast.Call]:
    ...
    that no longer exists while still passing.
    """
...
def test_whole_tree_mode_reports_without_enforcing(gate: ModuleType, whole_tree: list) -> None:
    ...
    Diff mode exists only to keep the landing PRs green: a whole-tree gate would
    fail every PR until the last site is converted and charge the break to
    whoever pushed next.
    """
