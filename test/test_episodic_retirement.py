"""Supersession must be bounded, precise, and reversible.

``_retire_stale_episodic`` tombstones episodes that reference a value the semantic
store just superseded. It is the only rule in the memory engine where a SIMILARITY
JUDGEMENT deletes, and it fires once per key per consolidation cycle, forever — so
its blast radius compounds silently. Measured on a store hours old: 21 of 101
episodes already retired, 14 of them by this rule.

Three properties make that acceptable, and each is pinned here:

* BOUNDED — at most ``_MAX_EPISODIC_RETIRED_PER_WRITE`` rows per call.
* PRECISE — a candidate must textually contain the superseded value, not merely
  score above the cosine bar. This is what separates "restates the value that is now
  false" from "is about the same topic".
* REVERSIBLE — the row keeps its text and nothing hard-deletes an episode, so
  ``get_retired_episodic`` shows what went and ``restore_episodic`` brings it back.

The vector arm is the one that matters and the one a plain test never reaches: with
no embedder ``_try_embed`` answers None and only the narrow text fallback runs, so a
test that does not force the vector arm exercises none of this. Every test below
stubs the embed and the search, which is also what makes the cosine score an INPUT
rather than a property of whatever embedder the host happens to have.
"""
...
_RESTATES = (
    "User prefers red for the theme",
    "the red option was chosen",
    "we set it to red",
    "red is the accent colour",
)
_SAME_TOPIC = (
...
def store(tmp_path: Path):
    ...
    """
    st = VectorMemoryStore(db_path=tmp_path / "memory.db")
    st.init()
    try:
...
class TestSupersessionIsPrecise:
    ...
    def test_the_match_ignores_case_and_json_quoting(self, store: VectorMemoryStore) -> None:
        ...
        rather than loud, so it needs its own assertion.
        """
        with _force_vector_arm(store):
...
class TestSupersessionSkipsTheWorkWhenNothingCanMatch:
    ...
    def test_the_probe_does_not_suppress_a_real_candidate(self, store: VectorMemoryStore) -> None:
        """The guard must be a superset, not a filter: a hit still reaches both arms."""
        with _force_vector_arm(store):
            store._retire_stale_episodic(_KEY, _OLD_VALUE)
        assert len(store.get_retired_episodic(limit=50)) == _MAX_EPISODIC_RETIRED_PER_WRITE
    ...
    def test_the_probe_is_case_insensitive_like_the_containment_test(
        self, store: VectorMemoryStore
    ) -> None:
        """Probed on the PRE-casefold value, because SQLite LIKE folds ASCII only.

        A folded needle can be WIDER than LIKE matches, which would make the guard
        narrower than the test it guards and silently stop retiring.
        """
        with _force_vector_arm(store):
            store._retire_stale_episodic(_KEY, '"RED"')
        assert len(store.get_retired_episodic(limit=50)) == _MAX_EPISODIC_RETIRED_PER_WRITE
