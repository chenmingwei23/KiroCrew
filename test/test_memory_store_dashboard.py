...
class TestAnAbsentParameterDoesNotFollowTheSessionKeyHeader:
    ...
    operator's own memory exactly where it was. ``api_memory_carve`` is the single
    exception and has its own class below: it consulted the binding already.
    """
...
class TestCarveAloneKeepsTheSessionBinding:
    """The one route whose contract IS "the silo you are bound to".

    It consulted ``_session_memory_store`` before this seam existed, so keeping that
    is parity rather than new reach -- and its own docstring is what documents why an
    unbound caller falls through to the v1 global store and is refused there.
    """

    @pytest.mark.asyncio
    async def test_carve_reads_the_bound_silo(self, env) -> None:
        env.declare(_FINANCE)
        assert env.tier(_FINANCE).set_semantic(_SILO_KEY, "vim", 1.0, "user_explicit") is None
        resp = await memory_handlers.api_memory_carve(
            _request("GET", "/api/memory/carve", env.state(bound_to=_FINANCE))
        )
        assert resp.status == 200
...
class TestARestoreNameStaysInItsStoresBackupDirectory:
    ...
    async def test_the_stores_own_backup_is_accepted(self, env) -> None:
        ...
        # The displaced file is moved aside, never overwritten: a restore is run by
        # someone who has already lost data once.
        assert _body(resp)["superseded"].startswith(f"{db_path.name}.superseded.")
        assert len(list(db_path.parent.glob(f"{db_path.name}.superseded.*"))) == 1
    ...
    async def test_a_backup_listing_never_carries_a_filesystem_path(self, env) -> None:
        ...
        env.declare(_FINANCE)
        db_path = env.seed_vector_file(_FINANCE, _SILO_KEY)
        assert memory_backup.backup_store(db_path) is not None
