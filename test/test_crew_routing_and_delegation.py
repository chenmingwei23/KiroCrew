...
class TestDelegationCarriesTheCrewsStore:
    ...
    @pytest.mark.asyncio
    async def test_preparing_a_store_never_takes_the_run_down(self):
        """Preparation, not a precondition.

        Every shape that reached this code before degraded the whole run: a
        builder without the step, one whose step is not awaitable, and one that
        raises. Each must cost the run its vector tier and nothing more.

        What holds this up is the ``except``, not the ``isawaitable`` check —
        awaiting a MagicMock raises, and the except catches it, so removing the
        type check alone keeps this green while removing the except does not.
        Both stay: the check is what keeps a legitimate sync implementation from
        being reported as an error every turn.
        """
        ...
        await prepare_store_vectors(object(), "coding")  # no ensure_store at all
        await prepare_store_vectors(MagicMock(), "coding")  # not awaitable
        await prepare_store_vectors(Raises(), "coding")  # raises
        await prepare_store_vectors(MagicMock(), "")  # no store named

    @pytest.mark.asyncio
    async def test_a_named_store_is_actually_stood_up(self, crew_config):
        ...
        try:
            ...
            assert "memory_stores/coding" in str(ctx_mod._vector_stores["coding"]._db_path)
        finally:
...
class TestCrewToCrewDelegation:
    """The ``delegate`` move: one crew handing work to another, on purpose.

    Delegating is the only move that changes which silo a topic runs in, so the
    interesting assertions are about the roster (who may be named) and the
    resolution (what a name buys). A crew the decision model invents must not
    become a dispatch, and the fallback must land the work where it already was.
    """
    ...
    def _orchestrator():
        ...
        return CrewOrchestrator.__new__(CrewOrchestrator)

    def test_the_roster_offers_the_other_crews(self, crew_config):
        roster = self._orchestrator()._delegation_roster("kirocrew")
        assert {r["crew"] for r in roster} == {"coding crew", "email crew"}
        assert {r["description"] for r in roster} == {"Owns the codebase", "Owns correspondence"}

    def test_a_crew_is_never_offered_itself(self, crew_config):
        """Delegating to yourself is the ``spawn`` move, and offering it invites
        a hop that changes nothing but costs a dispatch."""
        roster = self._orchestrator()._delegation_roster("coding crew")
        assert [r["crew"] for r in roster] == ["email crew"]

    def test_a_crew_without_triggers_is_not_a_delegation_target(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KIROCREW_HOME", str(tmp_path))
        (tmp_path / "config.json").write_text(
            json.dumps(
                {
                    "default_agent": "kirocrew",
                    "agents": {"kirocrew": {}, "quiet crew": {"description": "no triggers"}},
                }
            )
        )
        from kiro_crew.config import loader as loader_mod
        ...
        loader_mod._invalidate_config_cache()
        try:
            assert self._orchestrator()._delegation_roster("kirocrew") == []
        finally:
            loader_mod._invalidate_config_cache()

    def test_delegating_always_carries_the_targets_silo(self, crew_config):
        """The silo is what delegating buys, and it travels unconditionally."""
        orch = self._orchestrator()
        assert orch._resolve_delegate("coding crew")[1] == "coding"
        assert orch._resolve_delegate("email crew")[1] == "email"

    def test_a_declared_template_travels_and_an_undeclared_one_stays_empty(
        self, tmp_path, monkeypatch
    ):
        """The asymmetry is the config's. A crew that declares no ``kiro_agent``
        means "the install default", so reporting ``""`` is accurate; reporting a
        template it never named would be a claim the operator did not make."""
        monkeypatch.setenv("KIROCREW_HOME", str(tmp_path))
        (tmp_path / "config.json").write_text(
            json.dumps(
                {
                    "memory_stores": {"coding": {}},
                    "default_agent": "kirocrew",
                    "agents": {
                        "kirocrew": {},
                        "declared": {
                        ...
                        "undeclared": {"memory_store": "coding", "triggers": "draft"},
                    },
                }
            )
        )
        ...
        try:
            orch = self._orchestrator()
            assert orch._resolve_delegate("declared") == ("kirocrew", "coding")
            # Store still travels; template deliberately does not.
            assert orch._resolve_delegate("undeclared") == ("", "coding")
        finally:
            loader_mod._invalidate_config_cache()

    def test_the_two_crews_silos_differ(self, crew_config):
        orch = self._orchestrator()
        assert orch._resolve_delegate("coding crew")[1] != orch._resolve_delegate("email crew")[1]

    def test_an_invented_crew_runs_here_instead(self, crew_config):
        """Deny-by-default on the NAME, and the fallback direction matters.

        Returning ("", "") makes the dispatch a plain spawn, so the work stays in
        the crew already handling it. The opposite fallback -- picking some other
        crew, or the global store -- would put the task somewhere nobody chose,
        which is the leak the move exists to control.
        """
        assert self._orchestrator()._resolve_delegate("ghost crew") == ("", "")
        assert self._orchestrator()._resolve_delegate("") == ("", "")

    def test_a_broken_config_offers_no_delegation_and_does_not_raise(self, tmp_path, monkeypatch):
        """The orchestrator keeps handling work itself, which is what it did
        ...
        try:
            orch = self._orchestrator()
            assert orch._delegation_roster("kirocrew") == []
            assert orch._resolve_delegate("coding crew") == ("", "")
        finally:
            loader_mod._invalidate_config_cache()

    def test_the_decision_prompt_and_executor_both_know_the_move(self):
        """A move the prompt offers but the executor drops is silently ignored,
        and one the executor accepts but the prompt never mentions is dead."""
        import inspect

        from kiro_crew import crew_chat

        assert '"do": "delegate"' in crew_chat._DECISION_PROMPT
        src = inspect.getsource(crew_chat.CrewOrchestrator._apply)
        assert 'do in ("spawn", "delegate")' in src
        assert "delegate_store or slot.memory_store" in src, "the target's silo must win"
...
class TestTheSpawnEndpointActuallyDelegates:
    ...
    def _spawn_call(body: dict, cfg_payload: dict, tmp_path, monkeypatch):
        ...
        loader_mod._invalidate_config_cache()
        cov = pytest.importorskip("test.test_handlers_messaging_coverage")
        mgr = cov._mgr()
...
class TestASymlinkCannotAliasTwoCrewsOntoOneSilo:
    ...
    def aliased(self, tmp_path, monkeypatch):
        ...
        (finance.parent / "acme").symlink_to(finance, target_is_directory=True)
        yield finance
        loader_mod._invalidate_config_cache()
