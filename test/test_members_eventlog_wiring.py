...
class TestApiMembersProjections:
    ...
    async def test_rows_carry_projections_and_reconcile_is_idempotent(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.members.KiroCrewConfig.load", lambda: cfg
        )
    ...
    @pytest.mark.asyncio
    async def test_editing_model_appends_one_member_config_changed_model(self, tmp_path, monkeypatch):
        cfg = _fake_config({CREW: _agent(model="claude-x")})
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.members.KiroCrewConfig.load", lambda: cfg
        )
...
class TestHistoryRoute:
    ...
    async def test_newest_first_and_before_paging(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(_members_app(state))) as client:
            ...
            paged = await (
                await client.get(f"/api/members/{slug}/history?before=2")
            ).json()
...
class TestAgentPutConfigHook:
    def _app(self, state, cfg, monkeypatch):
        ...
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.agents.KiroCrewConfig.load", lambda: cfg
        )
