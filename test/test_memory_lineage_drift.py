...
#: A read keyword followed by its source. UPPERCASE-only, unlike the write scan.
#: Lowercase would collect ``from a`` and ``from past`` out of a ``logger.debug``
#: message and a prompt header, and the allowlist silencing those would grow into
#: a dictionary of English words. Every SQL statement in the module spells its
#: keywords in uppercase, and this scan is a completeness check rather than the
#: guard, so the trade lands the right way round.
_READ_RE: Final = re.compile(r"\b(?:FROM|JOIN)\s+" + _RELATION)
