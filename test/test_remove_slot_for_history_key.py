...
class TestAJobBornDuringTheDeleteIsNotStranded:
    ...
    async def test_an_unreadable_store_after_the_delete_names_the_gap(
        ...
        monkeypatch.setattr(
            sessions_module, "_owner_keys_bound_to_transcript", _second_pass_blind
        )
...
class TestALinkedKeyMustNameItsOwnTranscript:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "dashboard_slack_1785370133.085469", id="pre-migration-leftover-tab-file"
            ),
            pytest.param(
    ...
    async def test_a_dashboard_rows_stripped_stem_is_not_a_linkable_target(self, tmp_path):
        ...
        linkable set is the row's canonical stem alone, so an ordinary dashboard
        row is covered by the same rule as a channel-shaped one rather than by a
        separate gate that could be reasoned about independently.
        """
