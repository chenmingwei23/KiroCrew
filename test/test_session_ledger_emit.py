...
def test_a_leaked_turn_is_never_evicted_even_past_the_ceiling():
...
def test_a_wedged_writer_gives_up_after_the_cap_so_a_bounded_drain_finishes(caplog):
    ...
    with pytest.MonkeyPatch.context() as patch:
        ...
        assert emit.buffered_writes() == 0
        assert (
            emit.dropped_writes() == 4
        ), "the three entries and their failed aggregate marker were not counted exactly once"
    ...
    assert (
        emit.dropped_writes() == 6
    ), "the three entries and three failed marker passes were not counted exactly once"
