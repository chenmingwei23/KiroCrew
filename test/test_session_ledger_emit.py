...
def test_no_job_is_ever_submitted_without_a_session_key():
    ...
    def _spy(job, what, session_id):
        seen.append(session_id)
        return real(job, what, session_id)
...
def test_shutdown_retries_a_batch_waiting_out_its_backoff():
    """A retry schedule cannot outlive the process it is scheduled in.

    A failed append puts its batch into backoff. At shutdown, honouring that
    backoff does not defer the write -- the process is leaving, so nothing retries
    it later and the entries are simply lost. The shutdown drain therefore claims a
    batch inside its backoff and retries it against the remaining budget.

    The delay is pinned LONGER than the shutdown budget, which is what makes this
    deterministic rather than a race: a drain that waits the backoff out cannot
    write the entry within its timeout, and one that claims early writes it at once.
    No clock is asserted -- only which of the two happened.

    Mutation guard: making the shutdown drain honour the backoff reddens this.
    """
    calls = {"n": 0}
    real_append = lg.Ledger.append
    ...
    with pytest.MonkeyPatch.context() as mp:
        _open_session()
        assert emit.flush()
        # Far beyond any drain budget this test uses.
        mp.setattr(emit, "_retry_delay", lambda _attempts: 30.0)
        mp.setattr(lg.Ledger, "append", _flaky)
        emit.on_turn_started(SESSION, 1, "user")
        # The first pass fails and retains the batch, which is now parked for 30s.
        assert not emit.flush(timeout=0.5), "the injected failure did not retain a batch"
        assert emit.buffered_writes() >= 1, "the failed batch was not retained"

        assert emit.drain_for_shutdown(timeout=2.0), "shutdown abandoned a retained batch"

    assert "turn/started" in [e["type"] for e in _body()]
    assert emit.dropped_writes() == 0, "a retryable batch was counted as dropped"
