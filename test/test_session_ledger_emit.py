...
def test_retry_exhausted_loss_recovers_with_marker_first():
    _open_session()
    assert emit.flush()
    ...
    assert recovered[0]["type"] == "write/dropped", (
        f"first entry after retry loss was not marker: {[e['type'] for e in recovered]}"
    )
    assert recovered[0]["data"]["dropped_count"] == 1
    assert recovered[0]["data"]["dropped_bytes"] == 23
    assert recovered[0]["data"]["reasons"] == {
        "retry_exhausted": 1,
        "overflow": 0,
        "refused": 0,
    }
    assert recovered[0]["data"]["first_wall_time"] <= recovered[0]["data"]["last_wall_time"]
...
def test_a_lost_marker_is_merged_into_one_later_marker():
    ...
    emit._buffer(
        SESSION,
        emit._PendingJob(
            job=lambda: (_ for _ in ()).throw(OSError("initial loss")),
            what="initial loss",
            nbytes=17,
        ),
    )
    ...
    with pytest.MonkeyPatch.context() as patch:
        ...
        emit.on_turn_completed(SESSION, 1, stop_reason="lost-with-marker")
        assert emit.flush(timeout=20.0)
    ...
    assert markers[0]["data"]["dropped_count"] == 2, (
        f"marker loss did not merge original and blocked entry: {markers[0]['data']}"
    )
    assert markers[0]["data"]["reasons"] == {
        "retry_exhausted": 2,
        "overflow": 0,
        "refused": 0,
    }
...
def test_a_permanent_refusal_midbatch_owes_a_refused_marker():
    _open_session()
    assert emit.flush()
    ...
    body = _body()
    assert body[-2]["type"] == "write/dropped", (
