...
class TestPrivateStoreCallerIsolation:
    ...
    def test_the_http_route_carries_the_admitted_fence_verdict(self, tmp_path, monkeypatch):
        # The wiring, asserted at the route: without it the refusal above would
        # depend on a config record read after the body and the prewarms have
        # suspended, which is the window the carried verdict exists to close.
        import inspect

        from kiro_crew.dashboard.handlers import session_control as handlers

        source = inspect.getsource(handlers.api_session_control_create)
        assert "caller_fenced=_carried_fence(request)" in source
        assert "caller_fenced" in inspect.signature(sc.create_session).parameters
