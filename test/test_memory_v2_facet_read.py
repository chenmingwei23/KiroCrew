...
class TestTheAllowlistIsDerivedNotRestated:
    ...
    def test_the_carve_handler_dispatches_before_the_default_store_opens(self) -> None:
        """``carve`` names its own store, so it must not use the shared open.

        ``_memory_cmd``'s shared ``VectorMemoryStore()`` is hardwired to the default
        store's path. A verb whose target is a silo has to dispatch ahead of it, the
        same ordering the backup verbs rely on.
        """
        import inspect

        from kiro_crew import cli_commands

        assert callable(cli_commands._memory_carve)
        source = inspect.getsource(cli_commands._memory_cmd)
        before, _, after = source.partition("cfg = KiroCrewConfig.load()")
        assert "_memory_carve(args)" in before
        assert "_memory_carve" not in after
...
class TestTheFilterRidesTheIndexItClaims:
    ...
    def test_the_two_unindexed_axes_scan_and_the_docstring_says_so(
        ...
        sql, params = memory_schema.facet_page_query({facet: "x"})
        assert "idx_mi_" not in self._plan(crew_db, sql, params).replace("idx_mi_evict", "")
...
def _request(state: Any, query: dict[str, str], session_key: str = "dashboard:ui") -> Any:
...
class TestTheCarveRoute:
    """``GET /api/memory/carve`` — read-only, and bound to the CALLER's store.

    The route takes NO store parameter, and that is the security property rather
    than an omission: it resolves the silo from the caller's own recorded binding,
    so a session can only read the store it is bound to. A ``?store=`` parameter
    would let any dashboard or MCP caller read another crew's silo, which is what
    ...
    async def _call(self, handler_mod, store, query, *, silo: str) -> web.Response:
        """Drive the handler with *silo* as the caller's binding and *store* behind it.

        The binding is patched on ``_shared``, not on the handler module, because
        that is where ``resolve_requested_memory_store`` reads it. Patching the
        handler module would bind a name nothing calls, and the test would then
        assert against the REAL binding while reporting that it had chosen one.

        ``mock.patch.object`` context managers throughout, and never
        ``monkeypatch.undo()``: that reverts the conftest's ``KIROCREW_HOME`` pin
        along with the test's own patches and aims the next write at the
        ...
        with (
            mock.patch.object(_shared, "_session_memory_store", lambda *_a: silo),
            mock.patch.object(ContextBuilder, "ensure_store", staticmethod(_ensure)),
        ):
            return await handler_mod.api_memory_carve(_request(self._state(store), query))

    @pytest.mark.asyncio
    async def test_it_lists_the_callers_own_silo(self, handler, silo) -> None:
    ...
    async def test_a_silo_whose_vectors_cannot_be_stood_up_answers_503(self, handler) -> None:
        ...
        async def _ensure(_name: str):
            return None

        with (
            mock.patch.object(_shared, "_session_memory_store", lambda *_a: _FINANCE),
            mock.patch.object(ContextBuilder, "ensure_store", staticmethod(_ensure)),
        ):
            resp = await handler.api_memory_carve(_request(self._state(None), {}))
    ...
    async def test_a_store_query_parameter_is_refused_not_quietly_ignored(
        ...
        """
        resp = await self._call(handler, silo, {"store": _OPS, "count_by": "crew"}, silo=_FINANCE)
...
class TestTheCarveCliVerb:
    ...
    def test_an_undeclared_store_names_the_store_it_actually_read(self, capsys) -> None:
        """The name is resolved before it is reported.

        ``resolve_store_path`` degrades an undeclared name onto the default store,
        so echoing what was typed would attribute the default store's refusal to a
        store that was never opened.
        """
        _declare_silos(_FINANCE)
        self._run(store="nope", count_by="crew")
        printed = capsys.readouterr().out
        assert "'nope' is not declared" in printed
        assert f"Cannot carve {DEFAULT_MEMORY_STORE!r}" in printed
