...
import { quoteIntoDraft, useSelectionQuoteAsk } from '../chat-core/composer/selectionActions'
import {
  clearSideChatDrafts,
  consumeSideChatSeed,
  readSideChatDraft,
  seedSideChatDraft,
  subscribeSideChatDrafts,
  useSideChatDraft,
  writeSideChatDraft,
...
describe('sideChatDrafts store', () => {
  ...
  it('notifies subscribers on every write, and a mounted reader re-renders from the store', () => {
    const seen: string[] = []
    const off = subscribeSideChatDrafts(() => { seen.push(readSideChatDraft('A')) })
    writeSideChatDraft('A', 'x')
    seedSideChatDraft('A', 'y')
    off()
    writeSideChatDraft('A', 'not seen')
    expect(seen).toEqual(['x', 'x\n\n> y\n\n'])

    const { result } = renderHook(() => useSideChatDraft('A'))
    expect(result.current.text).toBe('not seen')
