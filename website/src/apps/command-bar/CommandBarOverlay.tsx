...
export default function CommandBarOverlay({
   ...
   *
   * ARTIFACTS ONLY. The sessions scope keeps the failure row it already had: its
   * text was the static `search_failed`, never a backend string, so nothing about
   ...
   * label.
   */
  const searchFailedText = artifactsFailed ? i18nT('apps.commandBar.artifact_search_failed') : ''
  /**
  ...
  const slotParts = (
    ...
    switch (slot.tag) {
      ...
      case 'retry-folders':
        // Says WHAT failed, not just "Retry": the folders view has no ErrorNotice
        // above its list — that notice belongs to the artifacts view, whose backend
        // string needed hiding — so this row is the only place the reader learns the
        // list could not be read. Same shape as the sessions row below, reached
        // through its own tag so that row stays byte-identical to main.
        return {
          icon: <RotateCcw size={14} className="lucide-inline" />,
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {/* ARTIFACTS ONLY: the sessions scope keeps the failure row it already had.
