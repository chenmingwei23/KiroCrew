...
function sessionRowIdentity(slot: Pick<Slot, 'key' | 'peer_id'>): string {
  return slot.peer_id ? `${slot.peer_id}:${slot.key}` : slot.key
}
    ...
    return (
      ...
      <motion.div ref={setPinnedDropRef} layout={rowAnimEnabled ? 'position' : false} layoutId={rowAnimEnabled ? `slot-${layoutScope}-${rowIdentity}` : undefined}
        ...
        <div ref={dndRow ? setNodeRef : undefined} {...(dndRow ? listeners : {})}
          ...
          title={
            // A peer-owned row does not open the transcript it names: there is no
            ...
            peerId
              ? i18nT('pages.chatSidebar.opens_the_instance_dashboard', { name: peerName })
              : undefined
          }
          ...
          onKeyDown={e => {
            ...
            e.preventDefault()
            if (!connected) return
            if (peerId) { onActivateRemote?.(peerId); return }
          ...
          onClick={e => {
            ...
            // Older Sessions already produce. A remote-EXECUTED local slot falls
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            <div className={`session-agent-label ${ROW_META_CLS} font-semibold truncate flex items-center gap-1 ${agentColor}`}>
              ...
              {peerId && <RemoteCrewChip name={peerName || ''} />}
              {peerId && (
                // WHERE THE CLICK ACTUALLY GOES, as VISIBLE text rather than a
                // hover `title` alone. The badge before it says "not on my
                // machine"; it does not say that activating the row leaves this
                // session behind, and the row is otherwise pixel-identical to a
                // remote-EXECUTED local row whose click DOES open the transcript.
                // Every sibling row's click means "open this session", so the
                // destination has to be readable BEFORE the click — and a `title`
                // never is on touch, nor until the pointer settles. The row's
                // `title` keeps the fuller "…, not this session" sentence; this is
                // the part that must not be hover-only.
                //
                // Same shape as the `answered_by` marker below for the same
                // reasons: plain muted text, not a second badge or a colour, so it
                // cannot read as an error on a row that works exactly as intended;
                // real text, so it lands in the accessible name in document order;
                // and shrinkable rather than `shrink-0`, so on a narrow rail it
                // ellipsizes instead of pushing the timestamp off the line. It
                // repeats the crew name the badge already carries, because a
                // pronoun ("opens its dashboard") stops being unambiguous the
                // moment two peers' rows sit next to each other.
                <span
                  data-testid="session-peer-destination"
                  className="min-w-0 max-w-[45%] truncate font-normal text-muted"
                  title={i18nT('pages.chatSidebar.row_opens_instance_dashboard', { name: peerName })}
                >
                  <span aria-hidden>{' · '}</span>
                  {i18nT('pages.chatSidebar.row_opens_instance_dashboard', { name: peerName })}
                </span>
              )}
              {agentDiverged && (
...
function ChatSidebar({
  ...
  const allRows = useMemo(
    () => (instanceSessions.rows.length === 0
      ? localSlots
      : [...localSlots, ...(instanceSessions.rows as unknown as Slot[])]),
  ...
  const runningSet = useMemo<Set<string>>(() => {
    ...
    return out
  }, [localSlots, workflowActiveSet, goalLoopSet])
