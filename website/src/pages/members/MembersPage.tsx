...
function MemberRow({
  ...
  const roster = useMemberProjection<RosterView>(m.slug, 'roster')
    ...
    <li key={view.name} className="group/row relative">
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[13px] font-medium truncate">{view.name}</span>
...
export default function MembersPage() {
  ...
  const activeRoster = useMemberProjection<RosterView>(active?.slug, 'roster')
  ...
  const orderedMembers = useMemo(() => {
    ...
    const names = sameMembership
      ...
      : [...members]
          .sort(
            (a, b) =>
              (tsByName.get(b.name) ?? 0) - (tsByName.get(a.name) ?? 0) || compareText(a.name, b.name),
          )
          .map((m) => m.name)
  ...
  // rendered here yet.
  const activeWake = useMemberProjection<WakeView>(activeSlug || null, 'wake')
    ...
    <div className="flex h-full min-h-0" data-testid="members-page">
      ...
      {active && (() => {
          ...
          <div className="mt-3 text-[11px] text-muted border border-border rounded-md px-2.5 py-2">
            {String((activeView ?? active).memory_store || 'default') === 'default'
              ? t('pages.membersPage.memory_shared_note')
              : t('pages.membersPage.memory_dedicated_note', {
                  store: String((activeView ?? active).memory_store),
                })}
          </div>
