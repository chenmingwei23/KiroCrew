...
import SidePanel, { SIDE_PANEL_MIN_W, SIDE_PANEL_RESERVED_W, type SidePanelLeadingTab } from '../chat/SidePanel'
import { useAnyLiveAppTab, usePanelTabs, type ViewKind } from '../../hooks/usePanelTabs'
import { usePanelTabDescriptors } from '../../hooks/panelTabRegistry'
import { usePanelDocumentActions } from '../chat/usePanelDocumentActions'
...
/** Chat-panel views this page cannot feed, withheld from the strip and the +
 *  menu (`SidePanel.hiddenViews`). Changes / Issues / Links read ChatPage's
 *  pull-request and link extraction over the transcript; Pins reads its pins
 *  query; none of that runs here, so each would render an affirmative "none".
 *  `summary` is the chat page's SESSION summary — withheld because next to the
 *  "Crew summary" chip it is an indistinguishable sibling label, not because it
 *  lacks data. Exported so the test pins the set. */
export const MEMBERS_UNFED_VIEWS: readonly ViewKind[] = ['changes', 'issues', 'links', 'pins', 'summary']
...
export default function MembersPage() {
  ...
  // withheld as well, per the binding rule above.
  const hiddenViews = useMemo<ReadonlySet<ViewKind>>(
    () =>
      new Set<ViewKind>(
        activeSlot
          ? MEMBERS_UNFED_VIEWS
          : [...MEMBERS_UNFED_VIEWS, 'files', 'artifacts', 'subagents', 'workflows', 'git', 'side', 'browser', 'logs', 'context'],
      ),
    [activeSlot],
  )
  ...
  // pay for a summary nobody is looking at.
  const summaryVisible = panelVisible && tabsCtl.activeId === CREW_SUMMARY_TAB_ID
  ...
  const activeSlotRef = useRef<string | null>(activeSlot || null)
  activeSlotRef.current = activeSlot || null
  ...
  const projectDir = useMemo(
    () => (activeSlot ? liveSlots.find((s) => s.key === activeSlot)?.project || undefined : undefined),
    [liveSlots, activeSlot],
  )
  ...
  return (
    ...
    <div className="flex h-full min-h-0" data-testid="members-page">
      ...
      {active && (() => {
          ...
          const overlayMotion = {
            initial: { x: '100%', width: 'auto', height: '100%', opacity: 1 },
            animate: { x: 0, width: 'auto', height: '100%', opacity: 1 },
            exit: { x: '100%', width: 'auto', height: '100%', opacity: 1 },
          }
          const motionFor = beside ? dockMotion : overlayMotion
          return (
            <AnimatePresence initial={false}>
              {panelMounted && (
                <motion.div
                  ...
                  initial={motionFor.initial}
                  animate={motionFor.animate}
                  exit={motionFor.exit}
                  transition={{ duration: 0.18, ease: [0.2, 0, 0, 1] }}
                  className={beside
                    ? 'h-full overflow-visible flex justify-end shrink-0'
                    /* The overlay MUST be dismissable, so it is the one placement
                       that hands the panel an onClose. On a phone the wrapper
                       spans the viewport so the panel's mobile `100%` width has
                       a definite box; on a tablet-width window the panel keeps
                       its own (resizable, persisted) width. */
                    : `fixed top-safe-offset-[42px] bottom-safe right-safe z-40 flex justify-end max-w-full bg-bg${isMobile ? ' left-safe' : ''}`}
