...
describe('MembersPage side panel (Crew summary tab) and edit jump', () => {
  ...
  it('a starred:true frame flips the Starred filter count and membership at page level without a roster refetch', async () => {
    ...
    // member must move the chip count and the filtered membership WITHOUT a
    ...
    const chip = await screen.findByTestId('member-filter-starred')
    // No stars yet: the count span is absent (rendered only when > 0).
