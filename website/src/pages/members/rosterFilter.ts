...
export const EMPTY_QUERY: RosterQuery = { search: '', starredOnly: false, source: 'all', status: new Set(), sort: 'recent' }
...
/** The rows to render: `members` sorted per `query.sort`, then narrowed by
 *  every active dimension (AND across dimensions, OR inside the status set). */
export function filterRoster<M extends RosterRowLike>(
  members: readonly M[],
  query: RosterQuery,
  signalsOf: (m: M) => MemberSignals,
): M[] {
  const q = query.search.trim().toLowerCase()
  return sortRoster(members, query.sort).filter(
    (m) =>
