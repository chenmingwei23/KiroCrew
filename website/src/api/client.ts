...
/**
 * The optional `store=` parameter every `/api/memory/*` route accepts.
 *
 * Returns the EMPTY string when the caller named no store, and that absence is
 * load-bearing: the gateway reads a missing parameter as "the caller's own
 * recorded binding" and applies the owner gate plus the unknown-name 404 only to
...
export const api = {
  ...
  memoryPreferences: (store?: string) => fetch('/api/memory/preferences' + memoryStoreQuery(store)).then(j),
  saveMemoryPreferences: (content: string, store?: string) => put('/api/memory/preferences' + memoryStoreQuery(store), { content }),
  memoryProjects: (store?: string) => fetch('/api/memory/projects' + memoryStoreQuery(store)).then(j),
  saveMemoryProjects: (content: string, store?: string) => put('/api/memory/projects' + memoryStoreQuery(store), { content }),
  memoryHistory: (store?: string) => fetch('/api/memory/history' + memoryStoreQuery(store)).then(j),
  ...
  /** Declare a new store in the config's `memory_stores` table and create its
   *  directory. 400 `invalid_memory_store_name`, 409 `memory_store_exists`.
   *  There is no delete counterpart: undeclaring a store would orphan its
   *  memory, which needs operator direction outside a dashboard button. */
  createMemoryStore: (name: string) =>
    post('/api/memory/stores', { name }).then(j) as Promise<{ ok: boolean; name: string }>,
  memoryRetired: (store?: string, limit?: number) =>
    fetch('/api/memory/retired' + memoryQuery({ store, limit })).then(j) as Promise<{ retired: RetiredMemory[] }>,
  ...
  memoryBackups: (store?: string) =>
    fetch('/api/memory/backups' + memoryStoreQuery(store)).then(j) as Promise<{ backups: MemoryBackup[] }>,
  ...
  vectorSemanticWrite: (key: string, value: string, store?: string) => put('/api/memory/semantic' + memoryStoreQuery(store), { key, value, source: 'user_explicit' }).then(j),
