...
| [rfc-mcp-lifecycle-event-log.md](rfc-mcp-lifecycle-event-log.md) | `draft` | **Blocked on a base.** The lifecycle-event package this RFC builds on (landed by [#3808](https://github.com/kirodotdev/KiroCrew/pull/3808)) is deleted: it had no emitter, no reader and no on-disk directory, and every one of its kinds names a fact the per-unit ledger owns as a `type`. So this RFC's three precedents — the ordering model, the writer, the failure posture — have no package to be precedents for; adopting them means restating them against `kiro_crew.ledger`, where ordering is already answered per unit. The #7366 capture points it names as emit sites are on an open PR, not on main |
