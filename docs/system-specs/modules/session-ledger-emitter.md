...
| `write/dropped` | writer recovery, before that session's next ordinary append | dropped count/bytes, retry-exhausted, overflow and refused counts, first/last observed wall time |
...
Retry exhaustion and a hard buffer-ceiling rejection both create one per-session pending-loss
record: `{dropped_count, dropped_bytes, reasons:{retry_exhausted,overflow,refused}, first_wall_time,
last_wall_time}`. A permanent refusal mid-batch -- a `LedgerError`, which is either a malformed
entry or a well-formed entry refused because another process owns the log -- also creates one, under
`refused`. `_permanent` cannot split those two causes (they share one exception type), so both mark:
the ownership refusal is an accepted fact this writer could not append and must not vanish silently,
and marking a malformed entry too costs one extra counted line rather than a hole. `dropped_bytes`
is the size hint the producer supplied for the rejected jobs;
the two wall times are epoch milliseconds bounding when this process observed the merged losses.
Both reason keys are present even when one count is zero.
...
next claimed batch. If more loss arrives while the marker is queued or retained, the counts, bytes,
reasons and time bounds merge into it. If the marker itself reaches the write-attempt ceiling, its
payload is folded back into the next marker along with ordinary jobs that could not pass it. The
session is not poisoned: a later append retries the marker and can resume once it lands.
