...
- **Revocation is re-decided at egress, not trusted from the link.**
  ...
  conversation id immediately after resolution — same 403 contract, same SEL
  shape, principal from the target spelling as `_deliver_channel_dm` does — so
  the decision moves later on that one path; it is never skipped. Every
  persisted-link caller keeps the default.
