...
| `write/dropped` | `{dropped_count, dropped_bytes, reasons:{retry_exhausted,overflow,refused}, first_wall_time, last_wall_time}` — one durable account of writer losses before later entries resume | yes |
...
facts are missing. Its two wall times are epoch milliseconds bounding when the writer observed the
merged losses, and both reason keys are always present, including when one count is zero. The next
drain for that session puts this marker ahead of every ordinary entry; more loss before it lands is
merged into the same marker, and a marker that is itself dropped carries its counts into the next
one. The `reasons` keys name why a job was lost: `retry_exhausted` (a transient failure retried to
its cap), `overflow` (a hard memory-ceiling rejection), and `refused` (a permanent `LedgerError` --
either a malformed entry or one refused because another process owns the log; the two share one
exception type and are not distinguished here).
...
documents, and that function is reused rather than copied. Failures are COUNTED: a unit that could
not be fully removed keeps its segments, so it still reads as a ledger and stays addressable by the
next pass, and it answers `failed` rather than being reported as collected.
...
Three things are skipped regardless of age, and each is a refusal rather than an oversight:
