...
it walks rows **least-recently-synced first** (`ORDER BY last_synced ASC` — the
column every successful ingest stamps, so a served source rotates to the back and a
deferred one rises; no separate cursor state), checks the remaining
