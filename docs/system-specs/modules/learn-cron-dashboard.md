...
(`monitor_patrol`, `autonudge_stop`, `set_project`): it drops the walk and accepts
...
agent, and its own `monitor_patrol` / `monitor_watch` is the normal way it
...
return. A refused `monitor_patrol` / `monitor_watch` therefore also appends a
...
replacement semantics, but the agent-facing `monitor_patrol` and `monitor_watch`
