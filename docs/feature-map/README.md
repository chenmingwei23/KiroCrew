...
| Crew Members | One durable pinned DM thread per crew member, read as a conversation (a chat-profile projection that hides tool/process rows behind a disclosure) with the member's escalations rendered as actionable cards — countdown, default action and selectable option replies (click to choose, then Send); the detail drawer lists the worker sessions the member is driving (live `slots` frames filtered on `created_by`) and the member's auto-patrol status — the nudge loop on its own `member-<slug>` slot (active / stopped with reason / none), with a roster avatar badge while a loop exists (accent while patrolling, warn once stopped); active patrols also list under Wake sources; the roster carries a per-crew star (persisted on the crew record) and persistent filters — starred-only and origin (mine / built-in / from packages) — so the package-installed crews the agent sync writes can be collapsed | `/members` — rail row when the crew preview is on; the open member rides `?member=<name>` (a bare `/members` opens the remembered member, else the first row; below md it stays the roster) — the escalation bell mirror deep-links the same param | `pages/members/MembersPage.tsx`, `components/autoNudgeLoop.ts` (shared cycle/countdown readouts), `pages/members/chatProjection.ts`, `pages/members/EscalationCard.tsx`, `pages/members/WorkingIndicator.tsx`, `pages/members/ProcessDisclosure.tsx`, `pages/members/ChatFoldRow.tsx`, `pages/members/chatProfileRenderers.tsx`, `pages/members/escalationState.ts`, `pages/members/useEscalationIndex.ts`, `pages/chat/EscalationNotice.tsx` (default escalation row on every other chat surface) | `handlers/members.py`, `handlers/agents.py` (`starred`), `slot_projection.py` (`created_by`), `handlers/autonudge.py` (registry read), `session_control.py` (`escalate_to_user`, the human as a peer), `src/kiro_crew/crew_conversation.py` (the per-member conversation index) | `GET /api/members`, `POST /api/members/{slug}/thread`, `GET /api/members/{slug}/activity`, `GET,PUT /api/members/{slug}/rules`, `GET /api/members/{slug}/conversation`, `PUT /api/agents/{name}` (`starred`), `GET /api/autonudge`, `GET /api/ws` (`slots`, `autonudge_state`), `POST /api/session-control/escalate` |
