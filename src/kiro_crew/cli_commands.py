...
def _memory_cmd(args: argparse.Namespace) -> None:
    ...
    store_name = getattr(args, "store", None) or DEFAULT_MEMORY_STORE
            ...
            if action == "list":
                ...
                if dropped and memory_store_version(store_name) != 2:
                    ...
                    print(
                        ...
                        "destination does not store facets, so the rows landed "
                        "WITHOUT that attribution; the export file still holds it."
                    )
                if "markdown" in data:
