...
def _memory_stores_to_scan() -> list[tuple[str, Path | None]]:
    ...
    for name in _declared_store_names():
        ...
        try:
            ...
            # CONFIRM the path belongs to the store we asked for. ``resolve_store_path``
            # DEGRADES rather than raising -- ``degrade_store_name`` falls back to the
            # default store, and ``_declared_stores`` degrades to just the default on any
            # config-read failure. The fresh ``KiroCrewConfig.load()`` above and that
            # memoized view can therefore disagree (a config save landing between the two),
            # and then this resolves the operator's OWN ``memory.db`` under a crew's name:
            # the global store gets scanned twice and every one of its findings is printed
            # as that crew's. That is exactly the misattribution this function's
            # attribution exists to prevent, so the positive predicate is the gate.
...
def scan_memory() -> list[dict]:
    ...
    Covers TWO tiers, and both are prompt-injected: the vector store's semantic and
    episodic rows, then every store's JSONL lessons file. The lessons tier is not
