...
def _config_snapshot_for_agent(agent_cfg) -> dict:
    ...
    the roster endpoint sends. Every other field is passed through as-is.
    """
    out: dict = {}
    for field in _CONFIG_FIELDS:
        value = getattr(agent_cfg, field, None)
        out[field] = bool(value) if field == "starred" else value
    return out
        ...
        for name, agent_cfg in agents.items():
            ...
            try:
                ...
                reconcile_member_config(
                    slug, name, agent_cfg, values.get(types.PROJ_ROSTER, {})
                )
                # Patrol closer.
