...
def test_a_file_only_in_the_staging_tree_is_invisible_to_the_approval_hash(
    tmp_path: pathlib.Path,
) -> None:
    """Pins the KNOWN GAP so it is a recorded measurement rather than an assumption.

    A path in staging that the source lacks contributes no row, so its bytes do not reach
    this value. The scenario is a file added to the source mid-copy, shipped by
    ``_copy_skill``, then removed from the source before the hash runs.

    Asserting the gap rather than the fix is deliberate. Hashing the union of both trees
    closes it and reddened four tests on the Windows shard: this value must EQUAL a pin
    ``_tree_hash`` took over the source alone, so a staged-only row cannot be added to one
    side of that comparison. When the gap is closed for real this test is the one to flip,
    and it names the constraint any fix has to satisfy.
    """
    mod = load_build()
    ...
    assert mod._staged_tree_hash(clean, source) == mod._staged_tree_hash(tainted, source)
...
def test_the_hash_equals_a_source_pin_for_a_legitimate_build(tmp_path: pathlib.Path) -> None:
    ...
    assert mod._staged_tree_hash(staged, source) == mod._tree_hash(source)
...
def test_a_binary_the_copy_drops_still_hashes_from_the_source(tmp_path: pathlib.Path) -> None:
    ...
    first = mod._staged_tree_hash(staged, source)
    assert first == mod._staged_tree_hash(staged, source)

    (source / "logo.png").write_bytes(b"\x89PNG\r\n\x1a\n\x00DIFFERENT")
    assert mod._staged_tree_hash(staged, source) != first
