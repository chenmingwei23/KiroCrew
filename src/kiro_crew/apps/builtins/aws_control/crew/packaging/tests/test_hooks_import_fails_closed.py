...
def test_the_prompt_ceiling_counts_bytes_not_characters(tmp_path: pathlib.Path) -> None:
    """A multibyte persona must be measured in BYTES, which is what the ceiling is named in.

    ``read(n)`` on a text stream bounds CHARACTERS. Measured: 1048576 three-byte characters
    is a 3145728 byte file, and ``len(data) > _MAX_PROMPT_BYTES`` reported it as within a
    1048576 byte limit -- so a CJK persona reached three times the ceiling in memory.

    ASCII cannot show this: one character is one byte, so an ASCII test passes either way.
    The content here is deliberately three bytes per character.
    """
    mod = load_build()
    agents = tmp_path / "agents"
    agents.mkdir()
    persona = agents / "persona.md"
    # Just over the ceiling in BYTES, comfortably under it in characters.
    chars = (mod._MAX_PROMPT_BYTES // 3) + 8
    persona.write_bytes(("\u4e2d" * chars).encode("utf-8"))
    assert persona.stat().st_size > mod._MAX_PROMPT_BYTES
    assert chars < mod._MAX_PROMPT_BYTES, "the fixture must be under the ceiling in chars"

    with pytest.raises(mod.ExportRefused) as caught:
        mod._read_text_openat(agents, pathlib.Path("persona.md"))
