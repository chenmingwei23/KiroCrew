...
#: and negotiates multipart itself), and change-detected by a content hash too -- computed
#: as the bytes are read for the upload, so it costs no extra pass -- behind a cheap
#: size+mtime pre-filter that decides whether to re-read at all. So this is the threshold
#: between the in-memory path and the streaming path, NOT a size above which a
...
#: writes. Memory does not scale with the largest file a cycle touches; it scales with the
...
class _HashingReader:
    """Wraps a binary file object and sha256's every byte read through it.

    ``upload_fileobj`` already reads the whole file to upload it, so hashing in the
    read path costs no extra I/O and no second pass -- a streamed file gets a real
    content hash for the price of the upload it was already doing. Only ``read`` is
    proxied because that is all ``upload_fileobj`` calls; the digest is read from
    ``hexdigest()`` after the upload returns.
    """
...
def _stream_upload_nofollow(
    ...
    This ALWAYS uploads: the decision to skip is made once, before the read, by
    ``_has_current_identity`` -- this function is not a second skip gate. The bytes
    are hashed as ``upload_fileobj`` reads them (``_HashingReader``, no extra pass),
    so it returns the content identity ``(size, hash, mtime_ns)`` for the caller to
    store, giving a streamed file the same task-produced identity a buffered one has.
    """
    if root is not None:
        fd = _open_nofollow_under(root, path)
    else:
    ...
    try:
        ...
        with os.fdopen(fd, "rb", closefd=False) as fh:
            ...
            store.put_stream(key, reader)
        return st.st_size, reader.hexdigest(), st.st_mtime_ns
    finally:
        os.close(fd)
...
def _streamed_copy_is_current(prev: "ObjMeta | None", size: int, mtime_ns: int) -> bool:
    """The fast path that lets a cycle skip RE-READING a large (streamed) file.

    Content identity is a hash, and the only way to know the current identity is to read the
    bytes. For a file small enough to read whole that is exactly what happens -- it is read,
    hashed, and the hash compared -- so no fast path is needed or wanted there: a live
    transcript is rewritten in place with its mtime RESTORED, so equal size and equal mtime
    do NOT prove equal content, and only the hash catches it (test_trap2). Trusting size+mtime
    for a file that is read anyway would reintroduce that miss.

    The fast path exists for the ONE case where reading is the cost being avoided: a file
    above the in-memory ceiling, streamed. There it is safe to trust size+mtime as a currency
    proof, but only against a stored identity THIS task produced by streaming (``prev.streamed``
    with a real ``hash``): a large versioned artifact is written once and not rewritten in
    place, so a matching size and mtime against a hash we computed means the same bytes.

    Everything else returns False -> read (or stream) and decide by hash:
      * ``prev is None`` or ``prev.hash == ""`` (SEEDED): no task-produced identity, so a key's
        mere presence never authorises a skip -- upload and establish one.
      * not ``prev.streamed``: a buffered entry is re-read and re-hashed; the hash decides.
      * size or mtime moved: possibly changed, so re-stream and re-hash.
    """
    return (
        prev is not None
        and prev.streamed
...
def run_backup_cycle(
    ...
    *,
    ...
    for local_path, rel_key, root in layout.iter_backup_files(settings):
        ...
        # ONE skip decision for every file. A file is skipped only when its stored CONTENT
        # IDENTITY (a hash this task produced) equals its current identity. Two paths reach
        # that one rule:
        #   * a large (streamed) file: the size+mtime fast path below can prove currency
        #     against a stored streamed hash WITHOUT re-reading it;
        #   * every other file: it is read and hashed, and the hash decides at the tail.
        # Neither key presence, nor size alone, nor the size ceiling ever concludes "skip" on
        # its own -- three findings in a row came from a second signal doing exactly that.
        #
        # The stat is a fast-path currency check only; it is not trusted as the file's
        # contents. When it authorises a skip the file is not read. Otherwise the read/stream
        # below re-opens under O_NOFOLLOW and re-validates, so a symlink swapped in after this
        # stat is refused at the read, not followed.
        try:
            st = local_path.stat()
        except FileNotFoundError:
            continue
        except OSError as exc:
            logger.warning("backup: cannot stat %s (%s); skipping this cycle", rel_key, exc)
            continue

        if _streamed_copy_is_current(prev, st.st_size, st.st_mtime_ns):
        ...
        try:
            ...
            # Too large to hold in memory as one bytes object, so stream it to the bucket
            # instead of reading it. boto3's upload_fileobj (via store.put_stream) transfers
            # in bounded chunks and negotiates multipart itself, so memory does not scale
            # with the file size and the only copy is NOT lost on task replacement. The agent
            # can write an artifact of any size into this tree, so this path is reachable in
            # ordinary operation, not an edge case.
            #
            # The skip decision was already made above by ``_has_current_identity``: reaching
            # here means the file must be uploaded, so the stream ALWAYS uploads and hashes as
            # it reads (no extra pass), and stores the content identity it produced. It does
            # not re-decide skip -- there is one gate, and this is not it.
            try:
                ...
                continue
            s_size, s_hash, s_mtime = streamed
            state.objects[rel_key] = ObjMeta(s_size, s_hash, streamed=True, mtime_ns=s_mtime)
            result.uploaded += 1
            result.uploaded_bytes += s_size
            continue
        except _Contended:
            result.deferred_locked += 1
            logger.debug("backup: %s locked, deferring to next cycle", rel_key)
        ...
        # Read and hashed, so decide by CONTENT IDENTITY: skip only if the freshly computed
        # hash equals the one stored for this key. This is what catches an in-place rewrite
        # that preserved size and mtime (a live transcript, test_trap2) -- the fast path could
        # not, which is why buffered files are never fast-pathed. A SEEDED entry has hash ""
        # and so never matches here, so a key merely present in the bucket is re-uploaded
        # rather than trusted (the write-once seeded-key finding). mtime is stored alongside
        # for the streamed fast path, but it is never the buffered skip signal.
        data_size = len(data)
        digest = hashlib.sha256(data).hexdigest()
        if prev is not None and prev.hash == digest:
            state.objects[rel_key] = ObjMeta(data_size, digest, mtime_ns=st.st_mtime_ns)
            result.skipped_unchanged += 1
            continue
        state.objects[rel_key] = ObjMeta(data_size, digest, mtime_ns=st.st_mtime_ns)
        store.put(layout.full_key(settings, rel_key), data)
        result.uploaded += 1
        result.uploaded_bytes += data_size
