...
class ObjMeta:
    size: int
    hash: str  # sha256 hex. "" only for a SEEDED entry (size known from a listing, contents
    #: unknown). A STREAMED entry now carries a real hash too, computed while the bytes were
    #: read for the upload -- see sidecar._HashingReader -- so change detection has content
    #: identity for every file this task has uploaded, buffered or streamed.
    #: True only for an entry this task STREAMED. Distinguishes a streamed entry (bytes this
    #: task uploaded) from a seeded one (an object a listing revealed), which matters because
    #: only a seeded entry has an empty hash and unknown mtime.
    streamed: bool = False
    #: Modification time in nanoseconds of the descriptor this task streamed, or 0 when
    #: unknown (seeded, or a buffered entry that does not use it). The streaming pre-filter
    #: compares size AND mtime_ns so an equal-length rewrite (same size, newer mtime) is
    #: re-streamed rather than skipped; the hash then confirms whether the new bytes actually
    #: differ. size alone was not enough -- a large file rewritten to the same length was read
    #: as unchanged and never uploaded.
    mtime_ns: int = 0
...
class BackupState:
        ...
        if isinstance(raw_objects, dict):
            for k, v in raw_objects.items():
                ...
                if not _finite_number(size):
                    continue
                digest = v.get("hash", "")
                streamed = v.get("streamed", False)
                mtime_ns = v.get("mtime_ns", 0)
                objects[k] = ObjMeta(
                    int(size),
                    digest if isinstance(digest, str) else "",
                    streamed=bool(streamed),
                    mtime_ns=int(mtime_ns) if _finite_number(mtime_ns) else 0,
                )
    ...
    def save(self, path: Path) -> None:
        payload = {
            # ``streamed`` is persisted, not derived: it is the difference between "this
            # task uploaded these bytes" and "a listing said an object of this size
            # exists", and dropping it on write means the next boot reads every streamed
            # entry as seeded and re-uploads it -- or worse, reads a seeded entry as
            # streamed and skips a changed file.
            "objects": {
                k: {
                    "size": m.size,
                    "hash": m.hash,
    ...
    def seed_sizes(self, sizes: dict[str, ObjMeta | int]) -> None:
        ...
        for key, meta in sizes.items():
            ...
            # ``streamed`` stays False: a listing reveals a size and nothing about how the
            # object got there, so this entry must not authorise the streaming path's
            # size-only skip.
