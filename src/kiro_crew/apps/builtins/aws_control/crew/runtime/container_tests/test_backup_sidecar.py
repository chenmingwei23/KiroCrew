...
def test_sidecar_seeds_from_bucket_to_avoid_reupload(tmp_path):
    # A restarted task seeds SIZES from the bucket listing, not hashes. A seeded key has no
    # task-produced content identity, so it is UNKNOWN and re-uploaded exactly once after the
    # restart to establish one -- key presence is not evidence this version was uploaded. The
    # next cycle has the hash and skips. This is the write-once seeded-key fix: the old code
    # skipped a seeded snapshot on presence and could keep stale bytes forever.
    s = make_settings(tmp_path)
    store = InMemoryObjectStore()
    ...
    # Fresh state (as after a restart) but same bucket. Two post-restart cycles: the seeded
    # entries are re-uploaded ONCE to gain a hash, then skipped -- not skipped on presence,
    # and not re-uploaded on every cycle.
    os.unlink(state_path(s))
    run_sidecar(s, store=store, max_cycles=2)
    assert store.put_count[snap_key] == 2, "a seeded snapshot must re-upload once to gain a hash"
    assert store.put_count[live_key] == 2, "the live body re-uploads once after a restart"
