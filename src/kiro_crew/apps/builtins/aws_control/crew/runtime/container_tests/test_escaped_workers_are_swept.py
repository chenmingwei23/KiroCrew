...
def test_the_sweep_takes_the_whole_process_group_not_just_the_leader() -> None:
    """F1: an escaped worker's own child must also be gone before the final backup.

    Signalling only the discovered leader pid leaves its child -- a process writing to the
    same filesystem the backup uploads -- alive. The sweep signals the leader's process
    GROUP and repeats discovery until nothing of ours is live, so the child dies too.
    """
    proc = subprocess.Popen(
