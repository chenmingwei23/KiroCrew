...
def test_a_seeded_entry_is_never_skipped_on_presence() -> None:
    """The unified predicate must not skip a key just because it exists in the store.

    The old write-once skip did (`if prev is not None and is_write_once_artifact(...)`),
    which kept stale bytes when a replacement task seeded a colliding key. The fix removes
    that presence skip: the only skip is content-identity equality, and a seeded entry has
    no task-produced hash, so it is uploaded. Pinned as a source rule so the presence skip
    cannot quietly come back.
    """
    src = pathlib.Path(sidecar_mod.__file__).read_text(encoding="utf-8")
    assert (
        "is_write_once_artifact(settings, rel_key)" not in src
    ), "the write-once presence skip is back; a seeded key can again suppress a changed file"
    # The fast path requires a real task-produced hash, so a seeded entry (hash "") cannot use
    # it, and the buffered tail only skips on hash equality -- neither skips on presence.
    assert 'prev.hash != ""' in src


def test_the_seeded_hash_is_empty_which_is_why_the_trade_exists() -> None:
    """A seeded entry still has no hash, which is why the pre-filter needs the streamed flag.

    A streamed entry now carries a real content hash, so an empty hash means SEEDED only.
    If ``store.list()`` ever returns a hash, seeding gains identity and this should be the
    thing that fails.
    """
    state_src = (pathlib.Path(sidecar_mod.__file__).parent / "state.py").read_text(encoding="utf-8")
    assert '"" only for a SEEDED entry' in state_src
...
# --- a seeded entry must not authorise the streaming skip -------------------


def test_a_seeded_entry_is_not_treated_as_streamed() -> None:
    """An empty ``hash`` had TWO meanings and the streaming skip trusted the wrong one.

    "Seeded from a bucket listing, contents unknown" and "streamed, deliberately not hashed"
    both stored ``hash == ""``. The streaming path's size-only skip is sound for the second
    and wrong for the first: a listing reveals a size and nothing about the bytes, so a file
    edited to the SAME length between two tasks was read as unchanged and never re-uploaded.
    ``open_slots.json`` has exactly that shape -- one slot removed, another added.
    """
    from container.backup.state import ObjMeta

    seeded = ObjMeta(1234, "")
    assert not seeded.streamed, "a seeded entry must not claim to have been streamed"

    streamed = ObjMeta(1234, "", streamed=True)
    assert streamed.streamed


def test_seed_sizes_never_marks_an_entry_streamed(tmp_path) -> None:
    """The seeding path is where the wrong claim would enter, so it is pinned there."""
    from container.backup.state import BackupState, ObjMeta

    state = BackupState()
    state.seed_sizes({"config/open_slots.json": 512, "data/x.bin": ObjMeta(99, "")})
    for key, meta in state.objects.items():
        assert not meta.streamed, f"{key} was seeded but marked streamed"


def test_the_flag_survives_a_round_trip(tmp_path) -> None:
    """Persisted, not derived: a boot that loses it falls back to the broken behaviour.

    Dropping it on write means the next boot reads a streamed entry as seeded (re-uploading
    it) or a seeded entry as streamed (skipping a changed file). The second is the data loss.
    """
    from container.backup.state import BackupState, ObjMeta

    path = tmp_path / "state.json"
    state = BackupState()
    state.objects["a"] = ObjMeta(10, "", streamed=True)
    state.objects["b"] = ObjMeta(20, "")
    state.objects["c"] = ObjMeta(30, "c" * 64)
    state.save(path)

    back = BackupState.load(path)
    assert back.objects["a"].streamed is True
    assert back.objects["b"].streamed is False
    assert back.objects["c"].streamed is False


def test_the_skip_reads_the_flag_not_the_empty_hash() -> None:
    """A source rule for the pre-filter, because the entries differ only in the flag.

    A behaviour test would need a real streamed upload and a real seeded entry of identical
    size, which needs the store fake to distinguish put from put_stream; reading the
    condition says the same thing without that machinery. The pre-filter must gate on
    ``prev.streamed`` AND size AND mtime -- size alone lost equal-length rewrites.
    """
    src = pathlib.Path(sidecar_mod.__file__).read_text(encoding="utf-8")
    assert "prev.streamed" in src, (
        "the streaming pre-filter no longer gates on prev.streamed, so a seeded entry can "
        "again authorise skipping a file whose size did not change"
    )
    assert (
        "prev.mtime_ns == mtime_ns" in src
    ), "the fast path is size-only again; an equal-length rewrite would be skipped"
    assert "_HashingReader" in src, "streamed files must be hashed during upload"
