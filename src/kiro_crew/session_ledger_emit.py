...
_pending: "OrderedDict[str, list[tuple[Callable[[], None], str]]]" = OrderedDict()
_pending_count = 0
...
def _submit(job: Callable[[], None], what: str, session_id: str) -> None:
        ...
        if _claim_inline(session_id):
            try:
                ...
                if failure is None:
                    ...
                    _drop(session_id, [(job, what)])
                else:
                    _retain(session_id, [(job, what)])
            finally:
...
def _retain(session_id: str, jobs: "list[tuple[Callable[[], None], str]]") -> None:
...
def _drop(session_id: str, jobs: "list[tuple[Callable[[], None], str]]") -> None:
...
def _backoff_ready_at_locked(state: "_RetryState") -> float:
    ...
    delay into a guaranteed drop at exactly the wrong time. Clamping instead gives
    the filesystem the most time the budget contains and then makes one attempt.
    """
    if not _draining_for_shutdown or _shutdown_deadline <= 0.0:
        return state.not_before
    latest = _shutdown_deadline - _MIN_SHUTDOWN_RETRY_GAP
    return min(state.not_before, latest)
...
def _drain_once() -> None:
    ...
    now = time.monotonic()
    with _lock:
        claimed: "list[tuple[str, list[tuple[Callable[[], None], str]]]]" = []
...
def _write_batch(session_id: str, jobs: "list[tuple[Callable[[], None], str]]") -> None:
    ...
    """
    landed = False
    for index, (job, what) in enumerate(jobs):
        failure = _run_job(job, what)
        if failure is None:
            landed = True
            continue
...
def drain_for_shutdown(timeout: float = _SHUTDOWN_DRAIN_SECONDS) -> bool:
    ...
    """
    global _draining_for_shutdown, _shutdown_deadline
    with _lock:
        ...
        _shutdown_deadline = time.monotonic() + timeout
...
def _fits_one_line(text: str) -> bool:
    ...
    *,
    ...
    if _fits_one_line(text):
        ...
        for piece in _text_slices(text):
            entry = ledger.append(
                "message/chunk",
                {"turn": int(turn), **({"step": int(step)} if step else {}), "delta": piece},
                src=src,
                ignorable=True,
            )
            cited.append(entry.seq)
        data["chunks"] = cited
        data["chars"] = len(text)
...
def on_message_received(
    ...
    *,
    ...
    _submit(_job, "appending message/received", session_id)
...
def on_message_sent(
    ...
    *,
    ...
    _submit(_job, "appending message/sent", session_id)
...
def on_message_steered(session_id: str, turn: int, *, mode: str = "", text: str = "") -> None:
    ...
    _submit(_job, "appending message/steered", session_id)
