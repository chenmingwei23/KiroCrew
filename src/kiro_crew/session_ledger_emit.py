...
class _PendingLoss:
    ...
    reasons: dict[str, int] = field(
        default_factory=lambda: {"retry_exhausted": 0, "overflow": 0, "refused": 0}
    )
    first_wall_time: int = 0
    last_wall_time: int = 0

    def merge(self, other: "_PendingLoss") -> None:
        """Fold *other* into this marker without losing its time bounds."""
        ...
        if not self.first_wall_time or (
            other.first_wall_time and other.first_wall_time < self.first_wall_time
        ):
            self.first_wall_time = other.first_wall_time
        if other.last_wall_time > self.last_wall_time:
            self.last_wall_time = other.last_wall_time
...
def _ledger() -> Any:
    ...
    launch with ``KIROCREW_SESSION_LEDGER`` unset never imports it, because
    ``enabled()`` refuses before any of those paths is taken.
    """
    global _subsystem
    if _subsystem is None:
        from kiro_crew import ledger
...
def _submit(
    ...
    *,
    ...
    """
    pending = _PendingJob(job=job, what=what, nbytes=nbytes, after=after)
...
def _buffer(session_id: str, pending: _PendingJob) -> None:
    ...
    """
    global _pending_count, _pending_high_water, _overflow_count
    overflow = False
    with _lock:
        would_count = _pending_count + 1
        would_bytes = _pending_bytes.get(session_id, 0) + pending.nbytes
        if would_count > _MAX_PENDING_COUNT or would_bytes > _MAX_PENDING_BYTES:
            _overflow_count += 1
            _record_loss_locked(session_id, [pending], "overflow")
...
def _retain(session_id: str, jobs: "list[_PendingJob]") -> None:
    ...
    if start_writer:
        _start_drain()
    if spent:
        _drop(session_id, jobs, loss_reason="retry_exhausted")
...
def _drop(
    ...
    *,
    ...
    with _lock:
        ...
        _record_loss_locked(session_id, jobs, loss_reason)
...
def _record_loss_locked(
    session_id: str,
    jobs: "list[_PendingJob]",
    reason: str | None,
) -> None:
    """Merge lost *jobs* into one per-session marker. ``_lock`` held."""
    if reason is None and not any(job.loss is not None for job in jobs):
        return
    ...
    for job in jobs:
        ...
        loss.reasons[reason] = loss.reasons.get(reason, 0) + 1
        if not loss.first_wall_time:
            loss.first_wall_time = now
        loss.last_wall_time = now
...
def _note_progress(session_id: str) -> None:
    ...
    if recovered:
        logger.warning(
            "session ledger writes for session %s are landing again; the "
...
def _write_batch(session_id: str, jobs: "list[_PendingJob]") -> None:
    ...
    for index, job in enumerate(jobs):
        ...
        if _permanent(failure):
            if job.loss is not None:
                _drop(session_id, jobs[index:], loss_reason="retry_exhausted")
                return
            # A permanent refusal is a LedgerError, which `_permanent` cannot
            # split into its two causes: a malformed entry the format rejected,
            # or a well-formed entry refused because another process owns this
            # log. The ownership case is a genuine hole -- an accepted fact this
            # writer could not append -- so it MUST owe a marker. The two share
            # one exception type and cannot be told apart here, so both mark:
            # over-marking a malformed entry costs one counted line, while
            # under-marking the ownership loss leaves exactly the silent hole
            # this record exists to prevent. Fail closed toward the marker.
            _drop(session_id, [job], loss_reason="refused")
            continue
        if landed:
            _note_progress(session_id)
...
def _report_overflow(session_id: str, total: int) -> None:
    ...
    """
    logger.error(
        "session ledger buffer full for session %s: the writer is too far behind "
    ...
    def _job() -> None:
        ...
        if _ledger().Ledger.exists(_KIND, session_id):
            ...
            if resumed and live_at_claim:
                logger.warning(
                    "session ledger %s: opened as a resume while turn %d is still "
...
def _entry_line_fits(entry_type: str, data: dict[str, Any], *, src: str) -> bool:
    """Whether *data* fits its complete serialized ledger entry.
