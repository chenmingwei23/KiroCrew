...
class _PendingJob:
    ...
    #: it. The file-CREATING record: refusing it means the ledger never exists, so
    ...
    #: whose ledger never came into being, so later entries for it count as loss
...
_dropped_count = 0
#: Sessions whose ledger CREATION failed permanently -- the file-creating record
#: was refused or spent its attempt budget, so no ledger file exists and no later
...
def _buffer(session_id: str, pending: _PendingJob) -> None:
    ...
    never be refused here: the record that CREATES a session's ledger file, and a
...
def drain_for_shutdown(timeout: float = _SHUTDOWN_DRAIN_SECONDS) -> bool:
    ...
    try:
        ...
        _report("draining the session ledger for shutdown", exc)
        drained = False
    if not drained:
        with _lock:
...
def _drop_oldest_live() -> None:
    ...
    if shed:
        logger.error(
            "session ledger is tracking more than %d turns in flight and shed %d "
    ...
    if len(_live) > _MAX_LIVE_TURNS:
        if not _live_overage_reported:
            ...
            logger.error(
                "session ledger is tracking %d turns in flight, over the %d "
...
def _handle(session_id: str) -> Any:
    ...
    None means exactly one thing: this session has no ledger file AND its creation
    was never attempted (feature off, never opened), so an event for it is
    deliberately not written rather than starting a ledger with no header. That is
    ...
    if not _ledger().Ledger.exists(_KIND, session_id):
        if creation_failed:
            # The file-creating record died permanently, so this session's ledger
            ...
            raise _ledger().LedgerError(
                "session ledger creation failed permanently; entry cannot be recorded",
...
def on_session_opened(
    ...
    *,
    ...
    def _flag_creation_failed() -> None:
        # The creating record died with no ledger file behind it: no later append
...
def on_session_closed(session_id: str, reason: str) -> None:
    ...
    def _forget() -> None:
        with _lock:
            ...
            # successor reusing the id creates its own ledger and must not
