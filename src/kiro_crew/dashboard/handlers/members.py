...
async def api_members(request: web.Request) -> web.Response:
    ...
    for row in rows:
        row["projections"] = projections.get(row["slug"], {"asOfSeq": -1, "values": {}})
...
async def api_member_history(request: web.Request) -> web.Response:
    ...
    return web.json_response({"slug": slug, "events": events, "lastSeq": last_seq})
