...
semantic rows, episodes, stats — and answers for whichever store the caller is
bound to. This module answers the operator's questions ABOUT the stores: which
ones exist, what is in each, what was retired out of one, what copies survive of
it, and declaring a new one.
...
def _taken_at(backup: Path) -> str | None:
    ...
    ``memory_backup.list_backups`` sorts by name. ``None`` for a name this format
    cannot parse, which the caller reports rather than guessing at.

    Reaches for ``memory_backup``'s own stamp format rather than re-typing it. The
    ...
    """
    try:
        stamp = datetime.strptime(backup.stem.rsplit(".", 1)[-1], memory_backup._STAMP_FORMAT)
    except ValueError:
        return None
    return stamp.replace(tzinfo=timezone.utc).strftime(_ISO_UTC_FORMAT)
...
async def _admin_store(
    ...
    return await resolve_requested_memory_store(request, state, operation)
...
def _store_unavailable(store: str) -> web.Response:
    ...
    up, and a name that no longer resolves to its own file — because the caller's
...
def _probe_store_blocking(name: str) -> dict[str, Any]:
    ...
    # resolve-then-confirm, not a bare resolve: ``resolve_store_path`` DEGRADES a
    # name the memoized config view no longer knows onto the DEFAULT store's
    # file, so counting through it would report the operator's own memory under a
    # silo's name.
...
async def api_memory_stores(request: web.Request) -> web.Response:
    ...
    resolves an absent parameter to (``MEMORY_STORE_ABSENT_GLOBAL``). The picker
...
async def api_memory_store_create(request: web.Request) -> web.Response:
    """POST /api/memory/stores — declare a store and create its directory.

    The name is validated by ``memory_store_name_defect``, the one predicate in
    the module that owns the shape rule, so this surface cannot admit a name the
    resolvers would refuse to compose a path for.

    The membership test that decides 409 is repeated INSIDE the locked write:
    the pre-check answers from a config snapshot taken before the lock, so two
    concurrent creates of one name would both pass it. Under the lock, the
    second sees the first's entry.
    """
    denial = await require_owner_dashboard_request(request, "memory.stores.create")
    if denial is not None:
        return denial
    ...
    if defect is not None:
        return web.json_response(
            {
                "error": f"invalid memory store name {name!r}: {defect}",
                "code": "invalid_memory_store_name",
            },
            status=400,
        )
    assert isinstance(name, str)  # memory_store_name_defect rejects every non-string
    if name in declared_store_names():
        return web.json_response(
            {
                "error": f"a memory store named {name!r} is already declared",
                "code": "memory_store_exists",
            },
            status=409,
        )

    # Populated by the mutate below when it finds the name already present, or
    # when the operator's own ``memory_stores`` value is not a table this can add
    # to. A list because the closure runs on a worker thread and only appends.
    refusals: list[str] = []
    ...
    def _apply(data: dict) -> dict | None:
        ...
        if not isinstance(stores, dict):
            # The operator wrote something else at that key. The loader ignores a
            # non-table there, so replacing it would silently erase their line —
            # and a store declaration is not worth destroying config to land.
            refusals.append("memory_stores_unreadable")
            return None
        if name in stores:
            refusals.append("memory_store_exists")
            return None
        # An empty entry, not a populated one: ``description`` and
        # ``embedding_provider`` both default to "" and are the operator's to set.
        stores[name] = {}
        data["memory_stores"] = stores
        return data
    ...
    try:
        ...
        logger.exception("Refusing to declare memory store %r: config unreadable", name)
        return web.json_response(
            {"error": "failed to read config file", "code": "config_unreadable"},
            status=500,
        )
    refused = refusals[0] if refusals else ""
    if refused == "memory_stores_unreadable":
        return web.json_response(
            {
                "error": "config.json's memory_stores is not a table; repair it before "
                "declaring a store",
                "code": "memory_stores_unreadable",
            },
            status=500,
        )
    if refused:
    ...
    await asyncio.to_thread(_create_store_dir, name)
    await _audit(request, "memory.stores.create", "success", name)
    return web.json_response({"ok": True, "name": name}, status=201)


def _create_store_dir(name: str) -> None:
    """Create *name*'s markdown root owner-only. Best-effort, and always AFTER
    the declaration lands.

    Order is load-bearing: ``ensure_memory_store_dir`` resolves through
    ``resolve_declared_store``, which DEGRADES an undeclared name onto the
    default store — so calling it before the config write would create nothing
    and return the operator's own workspace directory. The re-check here is what
    proves the declaration is visible to the resolver rather than assuming it.

    A failure is logged, not raised. The declaration is the durable half; the
    first open of the store creates this directory anyway, so turning a
    filesystem hiccup into a 500 would invite a retry that then 409s on the
    entry this call already landed.
    """
    if name not in declared_store_names():
        logger.warning(
            "memory store %r is not declared after its config write; leaving its "
            "directory to the first store open",
            name,
        )
        return
    try:
        created = ensure_memory_store_dir(name)
    except Exception:
        logger.warning("could not create the directory for memory store %r", name, exc_info=True)
        return
    if created.name != name:
        logger.warning(
            "memory store %r resolved its directory to %s, which is not its own; "
            "leaving it to the first store open",
            name,
            created.name,
        )
...
def _list_backups_blocking(db_path: Path) -> list[dict[str, Any]]:
    ...
    dropped rather than reported with a null size: it is genuinely no longer a
    backup the restore route could take.
    """
    rows: list[dict[str, Any]] = []
...
def _backup_now_blocking(db_path: Path) -> dict[str, int]:
    ...
    try:
        if memory_backup.backup_store(db_path) is None:
            ...
            return result
        result["backed_up"] = 1
        result["pruned"] = memory_backup.prune_backups(db_path)
...
def _restore_blocking(db_path: Path, backup: Path, store: str) -> str | None:
    """Restore *backup* over *store*'s file, reporting what was displaced.

    The displaced file is MOVED ASIDE by ``restore_from_backup`` rather than
    overwritten, and its name is what the caller reports back — a restore is
    performed by someone who has already lost data once, and the file being
    replaced may be the only copy of whatever it still held.

    The name is read by DIFFING the ``.superseded.*`` siblings across the call
    rather than by re-deriving the stamp, which would be a second spelling of a
    format ``memory_backup`` owns and would drift from it. ``None`` when nothing
    was displaced, which is the honest answer for a store that had no file yet.
    """
    pattern = f"{db_path.name}.superseded.*"
    before = {p.name for p in db_path.parent.glob(pattern)}
    memory_backup.restore_from_backup(backup, store)
    added = sorted({p.name for p in db_path.parent.glob(pattern)} - before)
...
async def api_memory_restore(request: web.Request) -> web.Response:
    ...
    **A restore takes effect on the next gateway start for a store this gateway
    already has open.** The displaced file is renamed, and an open SQLite
    connection follows the inode rather than the name, so the live
    ``VectorMemoryStore`` keeps reading and writing what was moved aside. There is
    no reopen to call: ``init()`` reassigns the connection with no idempotence
    guard, so calling it here would leak the old handle and could pull the
    database out from under a concurrent request — and it runs
    ``PRAGMA journal_mode=WAL``, which raises on exactly the corrupt file this
    route exists to replace. Resolving paths without opening the store is what
    keeps the recovery path reachable.
    """
    ...
    try:
        ...
        return web.json_response(
            {
                "error": "that backup fails its integrity check and was not restored",
                "code": "backup_corrupt",
            },
            status=409,
        )
    ...
    return web.json_response({"ok": True, "superseded": superseded})
