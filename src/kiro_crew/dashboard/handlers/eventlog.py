...
async def api_eventlog_events_get(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not grants.may_use_kind(app, kind):
            raise ContribError(
                "unit_kind_not_granted", f"this app may not read {kind} units"
            )
        ...
        if raw_limit != "":
            ...
            if limit < _LIMIT_MIN or limit > _LIMIT_MAX:
                raise ContribError(
                    "invalid_limit", f"limit must be {_LIMIT_MIN}..{_LIMIT_MAX}"
                )
    ...
    return web.json_response(
        {"kind": kind, unit.id_field: unit_id, "id": unit_id, "events": events, "lastSeq": last_seq}
    )
...
async def api_eventlog_events_post(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not grants.may_use_kind(app, kind):
            raise ContribError(
                "unit_kind_not_granted", f"this app may not append to {kind} units"
            )
...
async def api_eventlog_projection_put(request: web.Request) -> web.Response:
    ...
    _push_projection(request, unit, unit_id, key, result.row.value, result.row.seq, result.row.schema)
    _audit(
        app,
        "eventlog.publish",
        "granted",
...
def _push_projection(
    ...
    if broadcast is None:
        return
    payload: dict = {unit.id_field: unit_id, "key": key, "value": value, "seq": seq}
