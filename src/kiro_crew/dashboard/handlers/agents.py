...
def _rebind_crew_locked(
    ...
    nothing (GPT round-53); a vanished file raises ``FileNotFoundError``.
    """
...
def _foreign_private_copy_owner(crew: str, target: str) -> str | None:
    ...
    from under it. STRICT read (GPT round-54): a lenient read let a corrupt
...
def _reserved_binding_names(cfg_data: dict) -> set[str]:
    ...
    ``agent.default_agent`` (GPT round-53). Creating a spec file under any of
...
async def api_agent_fork(request: web.Request) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        def _create_record_bind() -> tuple[str, Path]:
            ...
            hold (GPT round-53): released between those steps, a locked bind
            ...
            otherwise capture the new copy (GPT round-51), and the legacy
            global fallback `agent.default_agent` is a resolvable reference
            like any binding (GPT round-53).
            """
            chosen: list[tuple[str, Path]] = []

            def _unwind(dest: Path, copy_name: str) -> None:
                # Locked writers cannot have bound the copy — we hold the
                # config lock — but a legacy `cfg.save()` writer bypasses it,
                # so the reference check re-reads the FILE before unlinking
                # (the round-50 posture, kept for that writer family).
                try:
    ...
    # failure set (GPT round-44). Outside the spec lock — the pass takes it
    ...
    async with _get_config_lock():
        ...
        def _create_published() -> Path:
            ...
            def _check_bindings_then_create(cfg_data: dict) -> None:
                ...
                # a resolvable reference like any crew binding (GPT round-53).
        ...
        def _undo_publish() -> None:
            ...
            # (GPT round-45): another crew can bind the just-created
            # destination before this rollback runs, and unlinking it then
            # breaks that crew's sessions with "Mode not found". The check
            # and the unlink are one critical section under the config lock,
            # so a binding writer cannot land between them. A RETAINED
            # destination is marked private to the publishing crew — the
            # publish did not commit, so the file must not surface as a
            # shared template.
            ...
            if outcome == "referenced":
                ...
                # deleting it or marking it private would break or
                # misattribute that crew's binding.
                return
            logger.warning("rollback could not remove %r; marking it private to %r", new_name, crew)
            with contextlib.suppress(Exception):
...
async def api_agent_reset(request: web.Request) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        try:
            ...
            # whole reset refuses instead (GPT round-53).
            return web.json_response(
                {"error": f"Origin template '{origin}' no longer exists", "code": "origin_missing"},
                status=409,
            )
