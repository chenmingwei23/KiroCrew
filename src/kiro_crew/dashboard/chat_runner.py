...
async def _start_next_queued_turn(state: DashboardState, slot: _ChatSlot) -> bool:
    ...
    # prefix ``is_subagent`` reads: the delivery crew log is content-keyed, so a row
...
async def _run_chat(
    ...
    *,
    ...
    # its reason steered in-band (see _refusal_notices); this crew log is what the
    ...
    try:
        ...
        async for event in event_stream:
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                if _refusal_notices:
                    # Same echo, same parser as the user-steer crew log: an
