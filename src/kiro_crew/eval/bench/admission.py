...
def f1_band(points: Sequence[SweepPoint], floor: float) -> tuple[float, float]:
    ...
    def describe(self) -> dict[str, object]:
        ...
        for kind in KINDS:
            ...
            out[kind] = {
                ...
                "balanced_f1_band_98": list(f1_band(bal_points, 0.98)),
...
def format_report(report: AdmissionReport) -> str:
    ...
    for kind in KINDS:
        ...
        bal_points = sweep(bal)
        band_low, band_high = f1_band(bal_points, 0.98)
        out.append(
            f"  under the 1:1 balance, F1 stays >= 0.98 for every threshold in "
            f"[{band_low:.2f}, {band_high:.2f}] — that view cannot pin a value"
        )
