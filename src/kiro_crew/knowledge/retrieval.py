...
class HybridRetriever:
    ...
    def _vector_search(
        ...
        # ``str | None``: the constructor guarantees a non-empty signature
        # whenever an embedder is wired, and a NULL bind here would match nothing
        # anyway (``embedding_sig = NULL`` is never true), so the widened element
        # type costs no safety.
        params: tuple[str | None, ...] = ()
        if self.embed_sig != ANY_EMBEDDING_SPACE:
            sql += " AND embedding_sig = ?"
            params = (self.embed_sig,)
        if source_id is not None:
            ...
            # Appended, never assigned: the signature predicate above may already
            # hold a placeholder, and rebinding here would bind its value to the
            # wrong ``?``.
