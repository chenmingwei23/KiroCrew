...
list is frozen at ``[1, 2, 3]``. There is no cutover, no dual write and no
backfill: the two lineages coexist for the life of each file.
...
consulted — is never reached for it. That is what makes "the operator's running
memory is untouched" a property of the code path rather than a claim a test
...
#: is authoritative: a lost or hand-edited stamp cannot misclassify a file.
