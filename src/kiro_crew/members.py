...
def record_activity(
    ...
    * ``"select_crew"`` — the orchestrator judged this member fits the task.
    ...
    try:
        ...
        svc = get_service()
        if dedupe_session and svc.has_participation(slug, member, session_key):
            # Asked of the WHOLE log, not a window of recent events: a busy member
            # pushes a session's record past any window, and the caller is asking
            # whether this participation fact is already recorded -- an answer that
            # can only see the tail says "no" and writes the duplicate.
            return False
