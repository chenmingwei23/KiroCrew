...
def _read_activity_checked(slug: str, limit: int = 0) -> tuple[list[dict], bool]:
    ...
    The second element is retained for callers that once had to fail closed on
    a partial read of the agent-writable file. The event log commits one line
    per append under fsync and the reader repairs a torn trailing line at load,
    so a partial-read completeness gap no longer arises here — the flag is
